...
describe('Utils', () => {
    ...
    it('get isExistShop', () => {
        ...
        Pools.add(poolExist);
        Pools.add(poolNoExist);

        const pIdt = Pools.findOne({ shop:'test.t' })._id;
        const pIdf = Pools.findOne({ shop:'test.f' })._id;

        expect(utils.isExistShop(pIdt, testShops)).to.equal(true);
        expect(utils.isExistShop(pIdf, testShops)).to.equal(false);
    });

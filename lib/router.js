...
Router.route('/pool/add', function() {
    let profileUser = Meteor.user().profile;
    if (!profileUser.username || !profileUser.phone || !profileUser.address) {
        throwNotification('error', 'Для создания пула нужно заполнить Имя, Телефон и Адрес');
        Router.go('/profile');
    }else {

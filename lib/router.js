...
Router.route('/user/:userId', function() {
    const apiHeatmapJs = IRLibLoader.load('//cdn.jsdelivr.net/cal-heatmap/3.3.10/cal-heatmap.min.js');
    Meteor.Loader.loadCss('//cdn.jsdelivr.net/cal-heatmap/3.3.10/cal-heatmap.css');
    this.wait(Meteor.subscribe('usersData'));
    this.wait(Meteor.subscribe('company'));
    this.wait(Meteor.subscribe('PoolsCompany', Meteor.user().profile.company));
    this.wait(Meteor.subscribe('OrdersCompany'));
    this.wait(Meteor.subscribe('OrdersItems'));
    if (apiHeatmapJs.ready() && this.ready()) {
        this.render('appUser');
    }else {
        this.render('loading');
    }

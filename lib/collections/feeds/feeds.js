...
class FeedsCollection extends Mongo.Collection {
     ...
     * @param poolId
     * @param data
     * @param ownerSilent - присылать ли уведомление владельцу пула
     */
    notifyEveryoneInPool(poolId, data, ownerSilent=true) {
        Orders.find({ poolId:poolId }).fetch().map(order => {
            data.ownerId = order.userId;
            data.seen    = ownerSilent && data.ownerId === data.userId;
            this.add(data);
        });
    }
    ...
    /**
     * @param data
     * @param ownerSilent - присылать ли уведомление владельцу
     */
    notifyEveryoneInCompany(data, ownerSilent=true) {
